...
class Window:
    def __init__(self, world, blueprint_library, vehicle, weather_controller):
        """
        创建车子的pygame窗口显示
        :param world: 世界对象
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:  # 窗口关闭
                    ...
                    if event.key == pygame.K_SPACE:
                        self.weather_controller.toggle_weather()
...
if __name__ == '__main__':
    ...
    window = Window(world, blueprint_library, vehicle, weather_controller)  # 创建窗口
