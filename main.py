                ...
                if scene_status == "干扰场景一":
                    if directions:
                        ...
                        if directions[0] == "right":
                            now_right_left_lane_info = get_now_road_car(self.vehicle, now_lane_flag=True, right_lane_flag=True)
                            now_lane_next_car_info = now_right_left_lane_info.get("now_lane").get("next_info")  # 前车信息
                            pang_lane_next_car_info = now_right_left_lane_info.get("right_lane").get("next_info")  # 旁前车信息
                            pang_lane_previous_car_info = now_right_left_lane_info.get("right_lane").get("previous_info")  # 旁后车信息
                        else:
                            now_right_left_lane_info = get_now_road_car(self.vehicle, now_lane_flag=True, left_lane_flag=True)
                            now_lane_next_car_info = now_right_left_lane_info.get("now_lane").get("next_info")  # 前车信息
                            pang_lane_next_car_info = now_right_left_lane_info.get("left_lane").get("next_info")  # 旁前车信息
                            pang_lane_previous_car_info = now_right_left_lane_info.get("left_lane").get("previous_info")  # 旁后车信息
                        ...
                        if pang_previous_flag and pang_next_flag and now_lane_next_car_info[0][1] < self.speed_limit:  # 满足变道条件
                    ...
                    if now_lane_next_car_info:
                        if now_lane_next_car_info[0][1] < 80:
                            print("请接管！！！！！！！！！！！！")
                # 获取前方道路
                waypoint = env_map.get_waypoint(self.vehicle.get_location()).next(max(1, int(get_speed(self.vehicle) / 3)))
                if waypoint:
                    waypoint = waypoint[0]
                else:
            ...
            while True:
                ...
                if not vices_car_list:  # 如果没有车流
                    ...
                    if self.whichOne + self.thread_car_number > len(vices_car_list):
                        ...
                        executor.submit(self.control_car, vices_car_list[self.whichOne: self.whichOne + self.thread_car_number])
...
def draw_line(location1=None, location2=None, locations=None, thickness=0.1, life_time=10, color=carla.Color(255, 0, 0)):
    """
    用直线连接两个点或者多个点
...
def create_vices(vehicle_traffic, vehicle):
    ...
    for i in range(number):  # 中一
        ...
        location = env_map.get_waypoint(vehicle_location).previous((i + 1) * previous_distance)[0].transform.location  # 后面车
        vice_locations.append(location + carla.Location(z=0.5))
    for i in range(number):  # 右一
        location = env_map.get_waypoint(vehicle_location).get_right_lane().next((i + 1) * next_distance)[0].transform.location  # 右一前车
        vice_locations.append(location + carla.Location(z=0.5))
        location = env_map.get_waypoint(vehicle_location).get_right_lane().previous((i + 1) * previous_distance)[0].transform.location  # 右一后车
        vice_locations.append(location + carla.Location(z=0.5))
    for i in range(number):  # 右二
        location = env_map.get_waypoint(vehicle_location).get_right_lane().get_right_lane().next((i + 1) * next_distance)[0].transform.location  # 右二前车
        vice_locations.append(location + carla.Location(z=0.5))
        location = env_map.get_waypoint(vehicle_location).get_right_lane().get_right_lane().previous((i + 1) * previous_distance)[0].transform.location  # 右二后车
        vice_locations.append(location + carla.Location(z=0.5))
    for i in range(number):  # 左一
        location = env_map.get_waypoint(vehicle_location).get_left_lane().next((i + 1) * next_distance)[0].transform.location  # 左一前车
        vice_locations.append(location + carla.Location(z=0.5))
        location = env_map.get_waypoint(vehicle_location).get_left_lane().previous((i + 1) * previous_distance)[0].transform.location  # 左一后车
        vice_locations.append(location + carla.Location(z=0.5))
    for i in range(number):  # 左二
        location = env_map.get_waypoint(vehicle_location).get_left_lane().get_left_lane().next((i + 1) * next_distance)[0].transform.location  # 左二前车
        vice_locations.append(location + carla.Location(z=0.5))
        location = env_map.get_waypoint(vehicle_location).get_left_lane().get_left_lane().previous((i + 1) * previous_distance)[0].transform.location  # 左二后车
        vice_locations.append(location + carla.Location(z=0.5))
...
def get_now_road_car(vehicle, now_lane_flag=False, left_lane_flag=False, right_lane_flag=False):
    ...
    if now_lane_flag:
        need_car_list["now_lane"] = [v for v in vice_cars if env_map.get_waypoint(v.get_location()).lane_id == env_map.get_waypoint(vehicle.get_location()).lane_id]  # 当前车道all车辆
    if left_lane_flag:
        need_car_list["left_lane"] = [v for v in vice_cars if env_map.get_waypoint(v.get_location()).lane_id == env_map.get_waypoint(vehicle.get_location()).get_left_lane().lane_id]  # 取左all车辆
    if right_lane_flag:
        need_car_list["right_lane"] = [v for v in vice_cars if env_map.get_waypoint(v.get_location()).lane_id == env_map.get_waypoint(vehicle.get_location()).get_right_lane().lane_id]  # 取右all车辆
...
def create_vehicle(vehicle_traffic, vehicle):
    ...
    locations = []  # 副车的坐标列表
    for i in range(3):
        location = env_map.get_waypoint(main_car_location).next(distance)[0].get_right_lane().transform.location  # 右前方五十米
...
if __name__ == '__main__':
    ...
    draw_arrow([start_location, start_end_location, start_end_location1])  # 划线
    vehicle_traffic = Vehicle_Traffic(world)  # 车辆创建对象
    # print(env_map.get_waypoint(start_location).next(1000)[0])
    ...
    while vehicle.get_location().distance(start_end_location1) > 10:  # 如果离终点小于十米就认为场景一结束
        sleep(1)  # 每间隔一秒判断是否到没有到终点

    # 干扰场景一
    destroy_vice(vehicle)  # 销毁除自车以外的所有车辆
    scene_status = "干扰场景一"
    main_car_control.autopilot_flag = True  # 转化为自动驾驶
    vices_car_list = create_vices(vehicle_traffic, vehicle)  # 创建副车
