...
import window_division
from disposition import *
from config import *

vices_car_list = []  # 所有副车列表
drive_status = "自动驾驶"  # 驾驶状态
directions = ["left", "right"]  # 变道顺序
                ...
                if scene_status == "干扰场景一":
                    if directions:
                        ...
                        if pang_previous_flag and pang_next_flag and now_lane_next_car_info[0][1] < self.speed_limit:  # 满足变道条件
                            right_left_lane(self.vehicle, direction=directions.pop(0))
                    else:
                        # print("没有变道方向了")
                        pass
                    ...
                    if now_lane_next_car_info:
                        if now_lane_next_car_info[0][1] < 50:
                            print("请接管！！！！！！！！！！！！")
                # 获取前方道路
...
if __name__ == '__main__':
    ...
    transform = carla.Transform(start_location, env_map.get_waypoint(start_location).transform.rotation)
    # 生成车辆
    vehicle = world.try_spawn_actor(blueprint_library.filter('*mini*')[1], transform)
    sleep(0.5)
    ...
    threading.Thread(target=window_division.window, args=(world, vehicle,)).start()
