    ...
    def follow_road(self):
        ...
        while self.flag:
            ...
            if self.autopilot_flag:
                drive_status = "自动驾驶"
                if keyboard.is_pressed('space'):  # 使用keyboard库来检测空格键是否被按下
                    self.autopilot_flag = False
                    message = "play"
                    self.data_recorder.record_mode_switch()
                    self.sock.sendto(message.encode(), (self.udp_ip, self.udp_port))
                    while keyboard.is_pressed('space'):  # 等待空格键释放，避免连续触发
                        pass
                # 瞬时速度
...
class Window:
    def __init__(self, world, blueprint_library, vehicle):
        ...
        spawn_point = carla.Transform(carla.Location(x=1.5, y = -0.21, z=1.15), carla.Rotation(pitch=7, yaw=0, roll=0))  # 传感器相对车子的位置设置
...
if __name__ == '__main__':
    ...
    vehicle_points = env_map.get_spawn_points()  
    vehicle_models = blueprint_library.filter('*mini*')  
